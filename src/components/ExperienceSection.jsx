        ...
        <div className="grid gap-6">
          {achievements.map((item, index) => (
            <motion.article
              ...
              className="relative rounded-[2rem] border border-white/70 bg-white/80 p-6 shadow-ambient backdrop-blur-xl dark:border-white/10 dark:bg-white/5 md:ml-14"
            >
              <div className="absolute -left-12 top-8 hidden h-6 w-6 rounded-full border-4 border-white bg-brand-500 shadow-lg md:block dark:border-slate-975" />
              <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                <span className="inline-flex w-fit rounded-full bg-slate-950 px-4 py-2 text-xs font-semibold uppercase tracking-[0.24em] text-white dark:bg-white dark:text-slate-950">
                  {item.year}
                </span>
